...
# PART 1: SYSTEM UTILS & REDIRECTOR
...
class UltimateSyncApp(ctk.CTk):
    def __init__(self):
        ...
        self.add_nav_btn("Help and Guides", "App Manual", "guide", color="#52796F")
    ...
    def add_nav_btn(self, text, desc, target, color=None):
        ...
        btn.grid(row=self.nav_row, column=0, padx=20, pady=(20, 0), sticky="ew")
        self.nav_row += 1
        ctk.CTkLabel(self.sidebar_frame, text=desc, font=ctk.CTkFont(size=11), text_color="gray").grid(row=self.nav_row, column=0, padx=20, pady=(0, 5), sticky="w")
        self.nav_row += 1
    ...
    def add_sub_btn(self, text, target):
        ...
        btn.grid(row=self.nav_row, column=0, padx=(35, 20), pady=3, sticky="ew")
        self.nav_row += 1
...
=== 2. DATA IMPORT ===
Go to CSV Setup and select your exported playlist file. The bot will standardize the columns and create a cleanlist.csv file for you.
...
Gaps Detected means run the Sync Engine again to let the bot fill any missed songs."""
    ...
    def find_and_verify(self):
        ...
        if not query or not os.path.exists("cleanlist.csv"): 
            alert_user("Error", "Please import a CSV first.", is_error=True)
            return
        ...
        if not match.empty:
            ...
            alert_user("Not Found", "No song found with that name.", is_error=True)
    ...
    def import_csv_logic(self):
        ...
        try:
            ...
            for col in df.columns:
                c = str(col).lower()
                if any(k in c for k in ['title', 'tittle', 'track', 'song', 'name']) and 'url' not in c: bt_col = col
            ...
            if bt_col and ba_col:
                ...
                alert_user("Error", "Could not find Title or Artist columns in your CSV.", is_error=True)
    ...
    def save_headers(self):
        ...
        if "cookie" in d:
            with open("browser.json", "w", encoding="utf-8") as f: json.dump(d, f, indent=4)
